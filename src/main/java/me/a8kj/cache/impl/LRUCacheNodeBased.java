package me.a8kj.cache.impl;

import java.util.HashMap;

import me.a8kj.cache.Node;
import me.a8kj.cache.service.CacheService;

/**
 * LRU (Least Recently Used) Cache implementation using a custom doubly linked
 * list and {@link HashMap}.
 * This cache automatically removes the least recently used entries when the
...
public class LRUCacheNodeBased<T, V> implements CacheService<T, V> {
     ...
     * replaced.
     * If inserting the new pair exceeds the cache's capacity, the least recently
    ...
    public void put(T id, V value) {
        if (cacheMap.containsKey(id)) {
            ...
            if (cacheMap.size() == capacity) {
                cacheMap.remove(tail.getPrevious().getKey());
    ...
    private void removeNode(Node<T, V> node) {
        ...
        node.getNext().setNext(node.getPrevious());
    }
}
